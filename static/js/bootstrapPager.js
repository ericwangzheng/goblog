var Pager = function (obj) {
    ...
    var str = '<nav><ul class="' + className + '">';
    if (firstButton) {
    ...
    for (var i = start + 1; i <= start + buttonSize; i++) {
        ...
        if (i == page) {
            str += '<li class="page-item active"><span class="page-link">' + i + '<span class="sr-only">(current)</span></span></li>';
        } else {
            str += '<li class="page-item"><a class="page-link" href="' + Pager.replaceUrl(pageParam, i) + '">' + i + '</a></li>';
        }
    }
    if (page >= max) {
